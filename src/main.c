...
// calc the init logpost and grad
...
int main(int argc, char *argv[])
{
    ...
    //
    // Alloc a N_beta*N_parm transit array for derivatives of N_beta chains
    //      N_beta * Nparm_dydx
    double ** transit_BetaParmdx_root;  
    transit_BetaParmdx_root = alloc_2d_double(n_ranks, N_parm);
    //
    /////////////////////////////////////////////////////
    //
    /////////////////////////////////////////////////////
    ...
    double * logpost_allranks_root;  
    logpost_allranks_root = alloc_1d_double(n_ranks);
    // calc the initial logpost_allranks_root
    mpi_init_calc_logllpp_grad(status, my_rank, n_ranks, root_rank, rootsent_tag, slavereturn_tag, transit_BetaParm_root, nline_data, data_NlineNdim, N_parm, logpost_allranks_root, transit_BetaParmdx_root);
    // 
    ...
    // NOTE only root needs transit_BP and logpost_allranks_root, slaves do not use them
    if (my_rank != root_rank) 
    {
        free_2d_double(transit_BetaParm_root);
        transit_BetaParm_root = NULL;
        free_2d_double(transit_BetaParmdx_root);
        transit_BetaParmdx_root = NULL;
        free_1d_double(logpost_allranks_root);
        logpost_allranks_root = NULL;
        ...
        for (int i=0; i<n_ranks; i++)
	{
	    ...
	    {
                printf("%d %f ", j, transit_BetaParmdx_root[i][j]);
	    }
            printf("\n");
	}
    }
